...
def extract_tool_use(input_text: str) -> Tuple[str, str, str]:
    ...
    # Modified to match Answer: Action, and match content that does not span lines
    # pattern = (
    #     r"\s*Thought: (.*?)\s*(?:Answer:.*?)?\s*Action: ([^\n\(\) ]+).*?Action Input: (\{.*\})"
    # )
    pattern = (
        r"(?:\s*Thought: (.*?))?\s*"  # Match optional Thought
        r"(?:Answer:\s*.*?\s*)?"      # Match optional Answer part
        r"Action: ([^\n\(\) ]+)\s*" # Match Action
        r"Action Input: (\{.*\})"     # Match Action Input
    )
...
class ReActOutputParser(BaseOutputParser):
    ...
    def parse(self, output: str, is_streaming: bool = False) -> BaseReasoningStep:
        ...
        1. If the agent needs to use a tool to answer the question:
            ```
            Thought: <thought>
            Action: <action>
