...
from core.agent.react.output_parser import ReActOutputParser
from core.agent.react.memory_pool import MemoryPool
...
def tell_llm_about_failure_in_extract_reasoning_step(
    ...
    """
    message = "Try again. Error: Could not parse output. Please follow the 'Thought-Action-Action Input' format or 'Thought-Answer' format."
...
class ReActAgentWorker(BaseAgentWorker):
    ...
    def compare_task_reasoning_masks(self, task, reasoning_step):
        """
        Extract and compare whether the mask lists in task.input and reasoning_step.action_input['mask'] are consistent.

        Parameters:
        - task: An object containing the input
        - reasoning_step: An object containing the action_input

        Returns:
        - True: The two masks are consistent
        - False: The two masks are inconsistent
        - None: Extraction failed
        """
        def extract_mask(input_text):
            """
            Extract the mask list from the text.
            """
            mask_pattern = r"\[([0-9,\s]+)\]"  # Match a sequence of numbers within square brackets
            match = re.search(mask_pattern, input_text)
            if match:
                try:
                    return list(map(int, match.group(1).split(',')))  # Convert to a list of integers
                except ValueError:
                    return None
            return None

        # Extract the mask from task.input and reasoning_step.action_input['mask']
        task_mask = extract_mask(task.input)
        reasoning_step_mask = reasoning_step.action_input.get('mask')

        # Check the extraction results
        if task_mask is None or reasoning_step_mask is None:
            return None

        # Return whether they are consistent
        return task_mask == reasoning_step_mask
    ...
    def _process_actions(
        ...
        try:
            ...
            # read memory pool
            reasoning_step.action_input, valid_read_memory_pool = memory_pool.get_action_input(reasoning_step)
            ...
            if reasoning_step.action not in tools_dict:          
            ...
            elif not valid_read_memory_pool:
                tool_output = self._handle_read_nonexistent_memory_pool_argument_name(reasoning_step, memory_pool, task)
            elif 'properties' in reasoning_step.action_input:
                tool_output = self._handle_wrong_action_input_format(reasoning_step)
            elif reasoning_step.action == 'drug_cell_response_regression_optimization' and self.compare_task_reasoning_masks(task, reasoning_step) == False:
                tool_output = self._handle_wrong_mask(reasoning_step, task)
            else:
                ...
                with self.callback_manager.event(
                    ...
                    try:
                        ...
                        # When there are no exceptions, save the parameters in the action input and the observation.
                        if tool and tool.metadata and tool.metadata.name:
                            memory_pool.save_action_input(reasoning_step)
                            memory_pool.save_observation(tool.metadata.name, tool_output.content)
    ...
    # def get_from_memory(self, reasoning_step):
    #     action_input = reasoning_step.action_input
    #     for k in action_input.keys():
    #         if k == 'drug_smiles':
    #             action_input[k] = 'C=C1C2=CCCC=C(NC(C)=CC34CC3COC=C(CCC13CC3)C4(C)CC)N2'
    #         if k == 'property':
    #             action_input[k] = 'logSolubility'
    #     reasoning_step.action_input = action_input
    ...
    def _handle_nonexistent_tool_name(
        ...
        with self.callback_manager.event(
            ...
            for error_char in error_chars:
                if error_char in reasoning_step.action:
                    content = f"Action again. Error: the Action should not contain `{error_char}`."
                    break
            else:
                content = f"Action again. Error: No such tool named `{reasoning_step.action}`."
            tool_output = ToolOutput(
                content=content,
    ...
    def _handle_read_nonexistent_memory_pool_argument_name(
        self, reasoning_step: ActionReasoningStep, memory_pool: MemoryPool, task
    ) -> ToolOutput:
        with self.callback_manager.event(
            CBEventType.FUNCTION_CALL,
            payload={
            ...
            # TODO(L10N): This should be localized.
            # import pdb; pdb.set_trace()
            if '(user_target_seq)' in reasoning_step.action_input.values():
                content = f"Action again. Error: Never give '(user_target_seq)', find target sequence in the origin user input."
            elif len(memory_pool.arguments) == 0:
                content = f"Action again. Error: MemoryPool is empty now, find action input in the origin user input."
            else:
                content = f"Action again. Error: In Action Input, you choose MemoryPool arguments that not exit, you should try to extract action input from user input, or choose from dict_keys of MemoryPool."
            content += f" The orginal user input: {task.input}"
            tool_output = ToolOutput(
                content=content,
                tool_name=reasoning_step.action,
                raw_input={"kwargs": reasoning_step.action_input},
    ...
    def _handle_wrong_action_input_format(
        self, reasoning_step: ActionReasoningStep
    ) -> ToolOutput:
        content = f"Action again. Error: The format of your Action Input is wrong, Never use AttributedDict."
        tool_output = ToolOutput(
            content=content,
            tool_name=reasoning_step.action,
            raw_input={"kwargs": reasoning_step.action_input},
    ...
    def _handle_wrong_mask(
        self, reasoning_step: ActionReasoningStep, task
) -> ToolOutput:
        content = f"Action again. Error: The mask in your Action Input is wrong, the length of this list is incorrect. Please use the mask in the orginal input: {task.input}"
        tool_output = ToolOutput(
            content=content,
            tool_name=reasoning_step.action,
            raw_input={"kwargs": reasoning_step.action_input},
    ...
    def get_llm_response(
        ...
        reasoning_step = cast(ActionReasoningStep, current_reasoning[-1])
        return reasoning_step.action, reasoning_step.action_input
    ...
    def _run_step(
        ...
        # memory prompt
        # if 'Observation' not in input_chat[-1].content:
        #     input_chat[-1].content += ' \n' + str(memory_pool)
        
        input_chat[-1].content += ' \n' + str(memory_pool)

        # import pdb; pdb.set_trace()
        print(f"input content: {input_chat[-1].content}")
        ...
        reasoning_steps, is_done = self._process_actions(
            task, tools, memory_pool=memory_pool, output=chat_response
        )
        # if reasoning_steps[0].action == 'drug_property_prediction':
        #     reasoning_steps[-1].observation = ['(gen_smiles)']
    ...
    def run_step(self, step: TaskStep, task: Task, memory_pool, **kwargs: Any) -> TaskStepOutput:
        """Run step."""
        return self._run_step(step, task, memory_pool)
