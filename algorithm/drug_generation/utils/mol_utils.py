...
def load_smiles_from_single_dataset(dataset='QM9'):
    ...
    df = pd.read_csv(f'./algorithm/drug_generation/data/{dataset.lower()}.csv')

    with open(f'./algorithm/drug_generation/data/valid_idx_{dataset.lower()}.json') as f:
...
def filter_smiles_with_labels(config, topk=3):
    ...
    if config.controller.base_model == "TransEDRP":
        df = pd.read_csv(f'./algorithm/drug_generation/{config.data.dir}{config.data.data.lower()}.csv')
