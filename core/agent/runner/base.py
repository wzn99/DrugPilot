...
class AgentRunner(BaseAgentRunner):
    ...
    def get_llm_response(
        ...
        action, action_input = self.agent_worker.get_llm_response(step, task)
    ...
    def _run_step(
        ...
        **kwargs: Any,
        ...
        if mode == ChatResponseMode.WAIT:
            cur_step_output = self.agent_worker.run_step(step, task, self.memory_pool, **kwargs)
    ...
    def _chat(
        ...
        mode: ChatResponseMode = ChatResponseMode.WAIT,
        stop_event: Optional[threading.Event] = None  # Added stop_event parameter for cancellation
    ) -> AGENT_CHAT_RESPONSE_TYPE:
        """Chat with step executor, supports stop_event for cancellation."""

        # If chat_history is provided, set it to memory
        ...
        while True:
            # Check if we need to stop, return None immediately if stop is requested
            ...
            # Exit loop if this is the last step
            if cur_step_output.is_last:
                result_output = cur_step_output
                break

            # Prevent infinite loops caused by tool_choice
            tool_choice = "auto"
    ...
    def chat(
        ...
        """执行聊天方法，带超时控制，超时后立即返回 None，并尝试中断。"""

        if tool_choice is None:
            tool_choice = self.default_tool_choice

        stop_event = threading.Event()  # 创建停止信号
        ...
        with self.callback_manager.event(
            ...
            with ThreadPoolExecutor() as executor:
                future = executor.submit(wrapped_chat)
                try:
                    return future.result(timeout=timeout)
                except TimeoutError:
                    stop_event.set()  # 触发停止信号
                    return None  # 立即返回 None



    @dispatcher.span
    @trace_method("chat_loop")
    def chat_loop(
        self,
        chat_history: Optional[List[ChatMessage]] = None,
        tool_choice: Optional[Union[str, dict]] = None,
        ...
        # 确保 chat_history 是 ChatMessage 对象的列表
        if chat_history is not None:
            chat_history = [
                ChatMessage(role="user", content=msg) if isinstance(msg, str) else msg
                for msg in chat_history
            ]
        else:
            chat_history = []

        # 添加系统消息到 chat_history
        system_prompt = self.get_prompts()['agent_worker:system_prompt'].template
        ...
        while True:
            ...
            message += " Don't forget to use the output format I have told you."

            # 将用户消息添加到 chat_history
            user_message = ChatMessage(role="user", content=message)
            chat_history.append(user_message)

            # 生成回复并添加到 chat_history
            with self.callback_manager.event(
                CBEventType.AGENT_STEP,
                payload={EventPayload.MESSAGES: [message]},
            ) as e:
                ...
                # 将工具的输出记录到 chat_history
                for tool_output in chat_response.sources:
                    ...
                    action = {} 
                    action['action'] = tool_output.tool_name
                    action['action_input']=tool_output.raw_input
