...
def train(model, train_data_loader, val_data_loader, test_data_loader, loss_fn, optimizer, n_epochs, device, scheduler):
    ...
    model_path = 'best_twosides_model.pth'  # Path to save the best model
    for epoch_i in range(1, n_epochs+1):
        ...
        if val_data_loader:
            with torch.no_grad():
                ...
                val_f1_score = val_metrics[3]  # Assuming F1-score is at index 3 of val_metrics
    
                ## Check if current validation F1-score is better and save model if true
                if val_f1_score > best_f1_score:
                    best_f1_score = val_f1_score
                    torch.save(model.state_dict(), model_path)
                    print(f'Saved new best model with F1-score: {best_f1_score:.4f}')
