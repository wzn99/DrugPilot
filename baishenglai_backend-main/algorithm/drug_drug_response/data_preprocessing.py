...
def atom_features(atom, atom_symbols, explicit_H=True, use_chirality=False):
    # to make sure the dimensions are equal for training and testing
    # atom_symbols = ['S', 'Ag', 'Sb', 'Al', 'Tc', 'Gd', 'Bi', 'Cl', 'Cu', 'Pt', 'Br', 'Mg', 'Cr', 'Hg', 'Zn', 'N', 'Ti', 'B', 'Au', 'Sr', 'C', 'Li', 'Co', 'O', 'Fe', 'Ca', 'Se', 'Si', 'K', 'H', 'Ga', 'Na', 'I', 'La', 'As', 'P', 'Ra', 'F']
...
def _normal_batch_rewrite( h, t, r, neg_size, data_statistics, drug_ids, seed):
    neg_size_h = 0
    neg_size_t = 0
    prob = data_statistics["ALL_TAIL_PER_HEAD"][r] / (data_statistics["ALL_TAIL_PER_HEAD"][r] + 
                                                            data_statistics["ALL_HEAD_PER_TAIL"][r])
    random_num_gen = np.random.RandomState(seed)
    ...
    return (_corrupt_ent_rewrite(data_statistics["ALL_TRUE_H_WITH_TR"][t, r], neg_size_h, drug_ids, random_num_gen),
            _corrupt_ent_rewrite(data_statistics["ALL_TRUE_T_WITH_HR"][h, r], neg_size_t, drug_ids, random_num_gen))  
...
def save_data_rewrite(data, filename):
    # dirname = f'tmp'
    # if not os.path.exists(dirname):
    #     os.makedirs(dirname)
    filename = filename
